...
)
{
    ...
    if (threadIdx.y == 0)
    {
        ...
        {
            scalar_t tmp = prefactors[i];
            buffer_prefactors[i] = tmp;
        }
    }
    __syncthreads();
...
template void sphericart_torch::prefactors_cuda<float>(int64_t, float *);
template void sphericart_torch::prefactors_cuda<double>(int64_t, double *);
