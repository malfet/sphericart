    ...
    bool hessian)
{
    ...
    bool result = adjust_cuda_shared_memory(scalar_size, l_max, GRID_DIM_X, GRID_DIM_Y, gradients, hessian);
    ...
    bool do_hessians)
{
    ...
    const int n_total = (calculator.l_max_ + 1) * (calculator.l_max_ + 1);
    ...
    else if (xyz.device().is_cuda())
    {
        ...
        {
            // just so CUDA memory checks don't complain.
            dsph = torch::empty(
                {1, 1, 1},
                torch::TensorOptions().dtype(xyz.dtype()).device(xyz.device()));
        }
        ...
        {
            throw std::runtime_error("Spherical harmonics only runs on float64 and float32 tensors");
        }
    }
    else
    {
